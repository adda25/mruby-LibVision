...
void LibVision::initParams() {
...
#if DEBUG_MODE
    std::cout << "LibVision-> alloc lbParams fails at: " << __LINE__
              << std::endl;
#endif
  }
  ...
  memcpy(this->lbParams->colorRange, defaultColorRange,
         sizeof(defaultColorRange));
...
void LibVision::loadImageFromMemory() {
#if DEBUG_MODE
  std::cout << "LibVision::setFramePathname --> " << lbParams->imagePath
            << std::endl;
#endif
  ...
  this->lastFrame =
      cv::imread(std::string(lbParams->imagePath), CV_LOAD_IMAGE_COLOR);
...
void LibVision::preprocessingFrameOTSU() {
  ...
  cv::threshold(lastGrayFrame, lastThrFrame, lbParams->otsuThresh, 255,
...
void LibVision::preprocessingFrameADPT() {
  ...
  cv::cvtColor(this->lastFrame, this->lastGrayFrame, CV_RGBA2GRAY);
  cv::adaptiveThreshold(lastGrayFrame, lastThrFrame, 255,
                        CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY_INV,
                        lbParams->adptThreshSize, lbParams->adptThreshMean);
}

void LibVision::setCurrentBackground() {
  if (this->lastFrame.rows == 0 || this->lastFrame.cols == 0) {
#if DEBUG_MODE
    LIB_VISION_DPRINT("setCurrentBackground -> lastFrame is empty");
#endif
    return;
  }
  cv::cvtColor(this->lastFrame, this->backgroundFrame, CV_RGBA2GRAY);
}

void LibVision::subtractBackground() {
  if (this->backgroundFrame.rows == 0 || this->backgroundFrame.cols == 0) {
#if DEBUG_MODE
    LIB_VISION_DPRINT("subtractBackground -> backgroundFrame is empty");
#endif
    return;
  }
  cv::Mat currentLastFrameGray;
  cv::cvtColor(this->lastFrame, currentLastFrameGray, CV_RGBA2GRAY);
  cv::bitwise_xor(currentLastFrameGray, this->backgroundFrame, opFrame);
#if DEBUG_WITH_IMAGES
  showImageForDebug(opFrame);
#endif
}

std::vector<std::vector<cv::Point> >
LibVision::findContours(int minContourPointsAllowed) {
...
#if DEBUG_MODE
    LIB_VISION_DPRINT(
        "no threshold frame, call preprocessingFrame_* --> return empty");
#endif
    return contours;
  }
  cv::Mat thrFrameCopy;
  lastThrFrame.copyTo(thrFrameCopy);
  cv::findContours(thrFrameCopy, allContours, CV_RETR_LIST,
...
std::vector<std::vector<cv::Point> >
LibVision::findCandidates(std::vector<std::vector<cv::Point> > contours,
                          uint vertexCount) {
  ...
  for (int i = 0; i < contours.size(); i++) {
    if (vertexCount == 1000) {
      ...
      if (approxCurve.size() == vertexCount &&
          cv::isContourConvex(approxCurve)) {
...
void LibVision::checkSquarePatterns() {
  ...
  cv::Mat patternImage =
      cv::imread(std::string(lbParams->patternImagePath), CV_LOAD_IMAGE_COLOR);
  ...
  for (int i = 0; i < this->candidates.size(); i++) {
    if (this->candidates[i].size() != 4) {
      continue;
    }
    cv::Mat warpThRoi = this->thresholdAfterWarp(
        this->getWarpPerspective(lastFrame, candidates[i]));
    int r = 0;
...
void LibVision::saveCandidates() {
  ...
  if (lbParams->polygonsFounds == 0) {
    ...
    lbParams->polygons =
        (Polygon *)realloc(lbParams->polygons, newSize * sizeof(Polygon));
  }
  ...
  for (int i = 0; i < (int)this->candidates.size(); i++) {
    ...
    lbParams->polygons[i + lbParams->polygonsFounds].polyPoints =
        (ScreenPoint *)malloc(numberOfPointsOfNewPoly * sizeof(ScreenPoint));
    ...
    lbParams->polygons[i + lbParams->polygonsFounds].numberOfPoints =
        numberOfPointsOfNewPoly;
    for (int j = 0; j < numberOfPointsOfNewPoly; j++) {
      lbParams->polygons[i + lbParams->polygonsFounds].polyPoints[j].x =
          this->candidates[i][j].x;
      lbParams->polygons[i + lbParams->polygonsFounds].polyPoints[j].y =
          this->candidates[i][j].y;
    }
  }
  // Save new state
...
void LibVision::drawCandidates(std::vector<std::vector<cv::Point> > candidates) {
  ...
  for (int i = 0; i < candidates.size(); i++) {
    for (int j = 0; j < candidates[i].size() - 1; j++) {
      cv::line(temp, candidates[i][j], candidates[i][j + 1],
               cv::Scalar(0, 0, 255), 2);
    }
    cv::line(temp, candidates[i][candidates[i].size() - 1], candidates[i][0],
             cv::Scalar(0, 0, 255), 2);
  }
  temp.copyTo(opFrame);
#if DEBUG_WITH_IMAGES
...
void LibVision::checkColor() {
  std::vector<std::vector<cv::Point> > newCandidates;
  cv::Scalar minCol =
      cv::Scalar(lbParams->colorRange[0], lbParams->colorRange[1],
                 lbParams->colorRange[2]);
  cv::Scalar maxCol =
      cv::Scalar(lbParams->colorRange[3], lbParams->colorRange[4],
                 lbParams->colorRange[5]);
...
bool LibVision::checkColorForRegion(std::vector<cv::Point> candidate,
                                    cv::Scalar minColor, cv::Scalar maxColor) {
  ...
  if (subframe.cols == 0 || subframe.rows == 0) {
    return false;
  }
...
cv::Mat LibVision::getWarpPerspective(cv::Mat roi,
...
cv::Mat LibVision::thresholdAfterWarp(cv::Mat roi) {
  ...
  cv::threshold(roi, patternCandidatesRoiThresh, DEFAULT_OTSU_THRESH, 255,
                CV_THRESH_BINARY_INV && CV_THRESH_OTSU);
  ...
  for (int i = 0; i < lbParams->polygonsFounds; i++) {
    ...
    for (int j = 0; j < lbParams->polygons[i].numberOfPoints; j++) {
      printf("--> Polygon index: %d vertexIndex: %d   \t x: %d \t y: %d \n", i,
             j, lbParams->polygons[i].polyPoints[j].x,
             lbParams->polygons[i].polyPoints[j].y);
    }
    printf("\n");
  }
}
...
void LibVision::debugPrintMethods() {
  ...
  for (std::map<std::string, LibVisionFunction>::iterator it =
           lbFunctions.begin();
       it != lbFunctions.end(); ++it) {
    std::cout << "--> " << it->first << "\n";
  }
}

void LibVision::testInterface() { std::cout << "Test succesful" << std::endl; }
...
void CLibVision_deinit(CLibVision_ptr rl) { delete LIB_VISION_CLASS(rl); }
