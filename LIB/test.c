...
void testCasePolygonsRecognitionCycle() {
	...
	for(i = 0; i < 150; i++) {
		...
		clvParams_ptr->imagePath = "testImage.png";
		clvParams_ptr->savedImagePath = "out.png";
	    CLibVision_requireOperations(clv, operation, (sizeof(operation) / sizeof(operation[0])) );
		char* operation1[1] = {"loadImageFromMem"};
	 
		CLibVision_requireOperations(clv, operation1, (sizeof(operation1) / sizeof(operation1[0])) );
		char* operation2[1] = {"preprocessingADPT"};
	    
		CLibVision_requireOperations(clv, operation2, (sizeof(operation2) / sizeof(operation2[0])) );
		char* operation3[1] = {"detectSquares"};
	    
		CLibVision_requireOperations(clv, operation3, (sizeof(operation3) / sizeof(operation3[0])) );
		char* operation4[1] = {"drawCandidates"};
	    
		CLibVision_requireOperations(clv, operation4, (sizeof(operation4) / sizeof(operation4[0])) );
		char* operation5[1] = {"saveFrame"};
	    
		CLibVision_requireOperations(clv, operation5, (sizeof(operation5) / sizeof(operation5[0])) );
		char* operation6[1] = {"preprocessingADPT"};
	    
		CLibVision_requireOperations(clv, operation6, (sizeof(operation6) / sizeof(operation6[0])) );
		
		char* operation7[1] = {"detectCircles"};
	    
		CLibVision_requireOperations(clv, operation7, (sizeof(operation7) / sizeof(operation7[0])) );
		
		char* operation8[1] = {"drawCandidates"};
	    
		CLibVision_requireOperations(clv, operation8, (sizeof(operation8) / sizeof(operation8[0])) );
	}
	// Use the LibVision Run 2
